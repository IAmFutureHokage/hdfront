...
export class StatsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (
      this.selectDate.before.getMonth() > 0 &&
      this.selectDate.before.getMonth() < 4
    ) {
      this.selectDate.from.setMonth(0);
    }
    if (
      this.selectDate.before.getMonth() > 3 &&
      this.selectDate.before.getMonth() < 7
    ) {
      this.selectDate.from.setMonth(3);
    }
    if (
      this.selectDate.before.getMonth() > 6 &&
      this.selectDate.before.getMonth() < 10
    ) {
      this.selectDate.from.setMonth(6);
    }
    if (
      this.selectDate.before.getMonth() > 9 &&
      this.selectDate.before.getMonth() < 13
    ) {
      this.selectDate.from.setMonth(9);
    }
    this.fieldsService
      .fetchAll()
      .then((response) => {
        this.infsysAll =
          response?.data?.filter((field) => field.cat === 'infsys') ?? [];
        this.channelsAll =
          response?.data?.filter((field) => field.cat === 'channels') ?? [];
        this.usersService
          .fetchAllUsers()
          .then((users) => {
            this.usersAll =
              users?.data?.filter(
                (user) => user.role === 'moder' || user.role === 'executor'
              ) ?? [];
            this.infsysAll.forEach((response) => {
              this.selectInfsys.push(true);
              if (response.name !== null)
                this.selectInfsysValue.push(response?.name);
            });
            this.channelsAll.forEach((response) => {
              this.selectChannel.push(true);
              if (response.name !== null)
                this.selectChannelsValue.push(response?.name);
            });
            this.usersAll.forEach((response) => {
              this.selectUsers.push(true);
              if (response.id !== null)
                this.selectUsersValue.push(response?.id);
            });
            this.colorsRand();
            this.requestsLoad();
          })
  ...
  statsMath(): void {
    ...
    this.requests.forEach((request) => {
      ...
      if (request.status === 5) {
        ...
        this.leadtime.treatment =
          this.leadtime.treatment +
          this.mathTime.closed.diff(this.mathTime.opened, 'minutes');
        this.leadtime.processing =
          this.leadtime.processing +
          this.mathTime.closed.diff(this.mathTime.processing, 'minutes');
      }
    ...
    if (this.amountStatus.closed > 0) {
      this.leadtime.treatment = Math.round(
        this.leadtime.treatment / this.amountStatus.closed
      );
      this.leadtime.processing = Math.round(
        this.leadtime.processing / this.amountStatus.closed
      );
    }

    this.chartOptions1 = {
      series: [
        this.amountStatus.opened,
        this.amountStatus.distributed,
        this.amountStatus.processing,
        this.amountStatus.closed,
      ],
    ...
    this.chartOptions5 = {
      ...
      colors: this.colors.slice(
        this.infsysAll.length + this.channelsAll.length
      ),
    };
  }
  ...
  requestsLoad() {
    this.requestsService
      ...
      .then((response) => {
        ...
        console.log(this.requestsdata.length);
        this.requestsFilter();
      })
      .execute();
  }
  ...
  requestsFilter() {
    ...
    this.channelsAll.forEach((item, index) => {
      if (this.selectChannel[index] && item.name !== null)
        this.selectChannelsValue.push(item.name);
    });
    this.selectInfsysValue = [];
    this.infsysAll.forEach((item, index) => {
      if (this.selectInfsys[index] && item.name !== null)
        this.selectInfsysValue.push(item.name);
    });
    this.selectUsersValue = [];
    this.usersAll.forEach((item, index) => {
      if (this.selectUsers[index] && item.id !== null)
        this.selectUsersValue.push(item.id);
    });

    this.requests = this.requestsdata.filter(
      (request) =>
  ...
  colorsRand() {
    ...
    for (
      var i = 0;
      i <
      this.infsysAll.length +
        this.channelsAll.length +
        this.usersAll.length +
        1;
      i++
    ) {
      this.colors.push(
  ...
  infsyscalc(a: string | null, b: string | null): number | void {
    if (a !== null && b !== null) {
      return this.requests.filter(
        (request) => a === request.chennal && b === request.infsys
      ).length;
    }
  }
